...
from layers.gating import SimplifiedLstm, BidirectionSLstm, VanillaLstm, BidirectionLstm, VanillaRNN, SimplifiedGRU, GatedRecurrentUnit, LstmNoPeepholes, LstmNOG, LstmNIG, LstmNFG, VanillaRNNDecoder, VanillaLstmDecoder 
from layers.layers import LinearLayer, SigmoidLayer, SoftmaxLayer
from layers.recurrent_decoders import ContextRNNDecoder
...
class DeepRecurrentNetwork(object): 
    ...
    def __init__(self, n_in, hidden_layer_size, n_out, L1_reg, L2_reg, hidden_layer_type, output_type='LINEAR', network_type='DNN', dropout_rate=0.0, loss_function='CCE'):
        ...
        for i in xrange(self.n_layers):
            ...
            if hidden_layer_type[i-1] in Encoder_variants:
                ...
                seq2seq_model = DistributedSequenceEncoder(rng, layer_input, self.d)
            ...
            if hidden_layer_type[i] == 'SLSTM' or hidden_layer_type[i] == 'SLSTME':
                ...
                hidden_layer = VanillaRNNDecoder(rng, layer_input, self.y, input_size, hidden_layer_size[i], self.n_out, p=self.dropout_rate, training=self.is_train)
                ...
                hidden_layer = SoftmaxLayer(rng, layer_input, input_size, hidden_layer_size[i]+1)
            else:
        ...
        if hidden_layer_type[-1] in Decoder_variants:
            ...
            if output_type == 'LINEAR':
                ...
                self.final_layer = SoftmaxLayer(rng, self.rnn_layers[-1].output, input_size, self.n_out)
...
#        self.L2_sqr = (self.W_hy ** 2).sum() 
    ...
    def multiclass_hinge_loss(self, predictions, targets, delta=1):
        ...
        return theano.tensor.nnet.relu(rest - corrects + delta).mean()
